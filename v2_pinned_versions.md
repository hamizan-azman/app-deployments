...
This document summarizes all dependency version changes made during V2 minimum version pinning across all 84 apps (67 deployed, 17 skipped).
...
## Task 2 New Apps (26 deployed)

Most Task 2 apps were installed from their upstream pyproject.toml or requirements.txt without modification. Pinning was applied in dockerfiles/<app>/requirements.txt where applicable. Apps that install via `pip install .` from pyproject.toml use the upstream's declared dependency ranges.

### Apps with notable dependency handling

| App | Approach | Notes |
|-----|----------|-------|
| vulnhuntr | pip install . | Added requests as undeclared runtime dependency |
| hackingBuddyGPT | pip install -e . | Editable install to preserve template files not declared as package_data |
| aider | pip install aider-chat (PyPI) | Built from PyPI instead of source due to setuptools-scm needing full git history |
| BabelDOC | pip install . | System deps: libglib2.0-0, libgl1, libgomp1 for OpenCV/ONNX |
| DeepGit | pinned requirements.txt | Converted toml>=0.10.2 to toml==0.10.2. Upstream Dockerfile had python:3.10, bumped to 3.11 per README |
| BruteForceAI | pinned requirements.txt | Removed sqlite3 (stdlib, not a pip package) |
| FinGenius | upstream requirements.txt | Fixed efinance package data dir permissions (chmod a+rw) |
| morphik-core | uv sync with CPU torch override | Lock file had CUDA torch deps. Pre-installed CPU torch via pip before uv sync. Replaced GHCR uv image with pip install uv |
| BiliNote | upstream requirements.txt | Backend-only deployment. ctranslate2 needs --security-opt seccomp=unconfined |
| BallonsTranslator | upstream requirements.txt | Pre-installed CPU-only PyTorch before main requirements |
| skyvern | uv pip compile from pyproject.toml | Replaced xdpyinfo package with x11-utils |
| hcaptcha-challenger | uv sync --all-extras | Replaced GHCR uv image with pip install uv. Needs xvfb-run wrapper |
| MoneyPrinterTurbo | upstream requirements.txt | Removed Chinese mirror fallback logic |
| ai-goofish-monitor | upstream requirements-runtime.txt | Removed Chinese PyPI mirror. Fixed vite output path (/dist not /web-ui/dist) |
| deepwiki-open | Poetry from pyproject.toml | Fixed COPY paths (removed apps/deepwiki-open/ prefix) |
| docetl | uv from pyproject.toml | Multi-stage build (python + node) |
| browser-use | uv sync from pyproject.toml | Upstream Dockerfile used as-is, installs chromium from apt |
| ChatDev | uv from pyproject.toml | Upstream multi-stage Dockerfile, backend only (port 6400) |
| pr-agent | pip from requirements.txt | Simplified from upstream multi-target Dockerfile to single target |
| chatgpt_telegram_bot | pip from requirements.txt | Python 3.8 (upstream pinned), all deps already ==pinned |
| Open-LLM-VTuber | uv sync from pyproject.toml | Upstream Dockerfile used as-is |
| Decepticon | pre-built images | No custom build. Compose-only deployment |

### Task 2 apps with no pinning changes needed

These apps either had all deps already pinned with == or were installed via pip install . with no version conflicts:
