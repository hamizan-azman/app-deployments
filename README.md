...
| # | App | URL | Method | Notes |
|---|---|---|---|---|
| 1 | [AiNiee](docs/non-docker/AiNiee_usage.md) | https://github.com/NEKOparapa/AiNiee | workstation | Cross-platform desktop GUI (PyQt5). Source install on Python 3.11. See docs/non-docker/AiNiee_usage.md. |
| 2 | [AI_NovelGenerator](docs/non-docker/AI_NovelGenerator_usage.md) | https://github.com/YILING0013/AI_NovelGenerator | workstation | Cross-platform desktop GUI (CustomTkinter). Source install on Python 3.11. See docs/non-docker/AI_NovelGenerator_usage.md. |
| 3 | [AppAgent](docs/non-docker/AppAgent_usage.md) | https://github.com/TencentQQGYLab/AppAgent | workstation | Workstation + connected Android device via ADB. End-to-end demo on a rooted Samsung Galaxy S9 over WSL2 + usbipd-win USB bridge. Required config edits: set `OPENAI_API_MODEL: "gpt-4o"` (default `gpt-4-vision-preview` is deprecated) and put a real key in `OPENAI_API_KEY`. Required ADB prep on rooted phones: `adb shell wm size reset` (parser crashes on display-size overrides). See docs/non-docker/AppAgent_usage.md. |
| 4 | [autoMate](docs/non-docker/autoMate_usage.md) | https://github.com/yuruotong1/autoMate | workstation | Upstream replaced the project we documented. v0.x was an OmniParser-based RPA tool with a Gradio UI at port 7888. v4.5.7 (the current version) is a "smart NAS for AI" MCP-over-HTTP tool hub. Same maintainer and same repo URL, completely different product. Install: `pip install 'automate-hub[full]'` from PyPI or `pip install '.[full]'` from source on Python 3.10+. See docs/non-docker/autoMate_usage.md (rewritten for v4.5.7) and finding 1. |
| 5 | [Cradle](docs/non-docker/Cradle_usage.md) | https://github.com/BAAI-Agents/Cradle | workstation | Windows 10/11 + NVIDIA GPU + target game/app on host. Optional ADB for mobile. Install on Python 3.10 (managed by uv) with `pip install -r requirements.txt`. Runtime requires a target game so verification stops at install. See docs/non-docker/Cradle_usage.md and finding 4. |
| 6 | [contextgem](docs/non-docker/contextgem_usage.md) | https://github.com/shcherbak-ai/contextgem | library | `pip install contextgem` resolves to v0.22.0 and imports cleanly on Python 3.12. See docs/non-docker/contextgem_usage.md. |
| 7 | [ExtractThinker](docs/non-docker/ExtractThinker_usage.md) | https://github.com/enoch3712/ExtractThinker | library | `pip install extract-thinker` resolves to v0.1.14 and imports cleanly on Python 3.12. See docs/non-docker/ExtractThinker_usage.md. |
| 8 | [GLaDOS](docs/non-docker/GLaDOS_usage.md) | https://github.com/dnhkng/GLaDOS | workstation | Workstation with mic + speaker. NVIDIA GPU recommended for usable latency. Install on Python 3.11 with `uv sync`. LLM backend can be pointed at any OpenAI-compatible endpoint via `LLM_BASE_URL` and `LLM_MODEL` env vars. See docs/non-docker/GLaDOS_usage.md. |
| 9 | [home-llm](docs/non-docker/home-llm_usage.md) | https://github.com/acon96/home-llm | plugin | Custom integration for Home Assistant 2025.7.0+. Installed via HACS. End-to-end exercised inside `homeassistant/home-assistant:stable` Docker with HACS 2026.4.4 plus a Local LLM Conversation Agent on `gpt-4o-mini` against the Generic OpenAI Compatible Conversations API backend. See docs/non-docker/home-llm_usage.md and finding 6. |
| 10 | [itext2kg](docs/non-docker/itext2kg_usage.md) | https://github.com/AuvaLab/itext2kg | library | `pip install itext2kg` resolves to v1.0.0 and imports cleanly on Python 3.12. See docs/non-docker/itext2kg_usage.md. |
| 11 | [pandas-ai](docs/non-docker/pandas-ai_usage.md) | https://github.com/sinaptik-ai/pandas-ai | library | `pip install pandasai` resolves to v3.0.0. Use Python 3.11. On Python 3.12 the install builds pandas from source and fails on a fresh system because the pandas pin has no 3.12 wheel. See docs/non-docker/pandas-ai_usage.md. |
| 12 | [UFO](docs/non-docker/UFO_usage.md) | https://github.com/microsoft/UFO | workstation | Windows 10/11 only. pywinauto + Windows UI Automation. Install on Windows Python 3.11.9 with `pip install colorama` first. `from ufo import ufo` imports cleanly. See docs/non-docker/UFO_usage.md. |
| 13 | [VideoCaptioner](docs/non-docker/VideoCaptioner_usage.md) | https://github.com/WEIFENG2333/VideoCaptioner | workstation | Cross-platform desktop GUI (PyQt5) + FFmpeg. PyPI install (`pip install videocaptioner`, v1.4.1) on Python 3.11. Source install uses `pip install .` (the upstream uses `pyproject.toml` and has no `requirements.txt`). See docs/non-docker/VideoCaptioner_usage.md. |
| 14 | [whispering](docs/non-docker/whispering_usage.md) | https://github.com/braden-w/whispering | workstation | Desktop app with microphone access (Tauri). Bare-metal install or prebuilt installer. Successor org is `EpicenterHQ/epicenter`. End-to-end exercised on Windows host with live mic and OpenAI Whisper API. See docs/non-docker/whispering_usage.md. |
| 15 | [Windrecorder](docs/non-docker/windrecorder_usage.md) | https://github.com/yuka-friends/Windrecorder | workstation | Windows-only. Continuous screen recording via DirectX. Install via `install_update.bat` (Poetry). UI launched by bypassing the broken `start_app.bat` and running `.venv\Scripts\python.exe -m streamlit run webui.py` directly. See docs/non-docker/windrecorder_usage.md and finding 5. |

## Non-Docker quick start

How to launch each of the 15 apps after the install steps in `docs/<app>_usage.md`.

| # | App | Start command | Access |
|---|---|---|---|
| 1 | [AiNiee](docs/non-docker/AiNiee_usage.md) | `python main.py` (or run the prebuilt installer) | Desktop window (PyQt5). Drag a folder into the window, configure source / target language in Settings, click **Start**. |
| 2 | [AI_NovelGenerator](docs/non-docker/AI_NovelGenerator_usage.md) | `python main.py` | Desktop window (CustomTkinter). Four tabs: Main Settings, Novel Settings, Writing, Logs. |
| 3 | [AppAgent](docs/non-docker/AppAgent_usage.md) | `python learn.py` (exploration mode) or `python run.py` (task mode) | Console prompt for app name plus ADB-controlled Android device. The agent screenshots the phone, reasons, and submits taps and swipes. |
| 4 | [autoMate](docs/non-docker/autoMate_usage.md) | `automate` | Browser at http://127.0.0.1:8765. Web chat plus a Settings page that copies an MCP install blob for OpenClaw / Claude Desktop / Cursor / Cline. |
| 5 | [Cradle](docs/non-docker/Cradle_usage.md) | `python runner.py --envConfig conf\env_config_<env>.json` (after `conda activate cradle` or sourcing the uv venv) | No separate UI. Drives the target game or app directly on the host display. Keep the target window focused and unminimised. |
| 6 | [contextgem](docs/non-docker/contextgem_usage.md) | `from contextgem import Document, DocumentAnalyzer` in your Python code | Python library, no standalone UI. Used inside other applications or scripts. |
| 7 | [ExtractThinker](docs/non-docker/ExtractThinker_usage.md) | `from extract_thinker import Extractor, Contract` in your Python code | Python library, no standalone UI. Used inside other applications or scripts. |
| 8 | [GLaDOS](docs/non-docker/GLaDOS_usage.md) | `uv run glados start` | Voice. Speak into the microphone, the assistant replies through the speaker. `Ctrl+C` to stop. |
| 9 | [home-llm](docs/non-docker/home-llm_usage.md) | Inside Home Assistant: **Settings -> Devices & Services -> Add Integration -> Local LLM** | HA chat panel and Voice Assistants. Wire it up via **Settings -> Voice Assistants -> Add Assistant** and pick the Local LLM agent. |
| 10 | [itext2kg](docs/non-docker/itext2kg_usage.md) | `from itext2kg import iText2KG` in your Python code | Python library, no standalone UI. Pass a list of text documents, get back a knowledge graph object. |
| 11 | [pandas-ai](docs/non-docker/pandas-ai_usage.md) | `from pandasai import SmartDataframe` in your Python code | Python library, no standalone UI. Wraps a pandas DataFrame and answers natural-language queries about it. |
| 12 | [UFO](docs/non-docker/UFO_usage.md) | `python -m ufo --task <task_name>` | Console prompt plus Windows UI Automation control of the target Windows application (Word, Excel, browser). Keep the target window visible. |
| 13 | [VideoCaptioner](docs/non-docker/VideoCaptioner_usage.md) | `videocaptioner` (PyPI install) or `python main.py` (source install) | Desktop window (PyQt5). Load a video, configure subtitle language and output, run the captioning pipeline. SRT export or burn-in. |
| 14 | [whispering](docs/non-docker/whispering_usage.md) | Launch the Whispering app from the prebuilt installer (or `pnpm tauri dev` for source) | Desktop window (Tauri) plus a system-wide hotkey (`Ctrl+Shift+.` on Windows by default). Press, speak, release, transcribed text lands in the clipboard. |
| 15 | [Windrecorder](docs/non-docker/windrecorder_usage.md) | `start_app.bat` (or `.venv\Scripts\python.exe -m streamlit run webui.py` if the bat fails on the host, see finding 5) | Browser at http://localhost:8501 (Streamlit). Tabs for Settings, Search, and Rewind. Click **Start Recording** to begin. |

## Cannot deploy (8)


| # | App | URL | Reason | Notes |
|---|---|---|---|---|
| 1 | [TaskMatrix](docs/non-docker/TaskMatrix_usage.md) | https://github.com/chenfei-wu/TaskMatrix | GPU server unavailable | NVIDIA GPU 16+ GB VRAM minimum (60+ GB for full set). See docs/non-docker/TaskMatrix_usage.md. |
| 2 | [MedRAX](docs/non-docker/MedRAX_usage.md) | https://github.com/bowang-lab/MedRAX | GPU server unavailable | NVIDIA GPU 12+ GB VRAM minimum. See docs/non-docker/MedRAX_usage.md. |
| 3 | [functionary](docs/non-docker/functionary_usage.md) | https://github.com/MeetKai/functionary | GPU server unavailable | NVIDIA GPU 24+ GB VRAM minimum. vLLM/SGLang backend. See docs/non-docker/functionary_usage.md. |
| 4 | [Linly-Talker](docs/non-docker/Linly-Talker_usage.md) | https://github.com/Kedreamix/Linly-Talker | GPU server unavailable | NVIDIA GPU 8+ GB VRAM minimum. SadTalker + Wav2Lip pipeline. See docs/non-docker/Linly-Talker_usage.md. |
| 5 | [droidrun](docs/non-docker/droidrun_usage.md) | https://github.com/droidrun/mobilerun | Upstream blocked | droidrun 0.5.9 imports `AsyncMobilerun` from the mobilerun package. The latest mobilerun (v0.6.0rc2) no longer exports it. Mid-migration. The repo was also renamed `droidrun/droidrun` to `droidrun/mobilerun`. See docs/non-docker/droidrun_usage.md and finding 3. |
| 6 | [wiseflow](docs/non-docker/wiseflow_usage.md) | https://github.com/TeamWiseFlow/wiseflow | Upstream blocked | Upstream rewrote the project as a TypeScript/Node codebase. The Python + Chromium + PocketBase install path the doc describes is stale. The doc needs a full rewrite before deployment. See docs/non-docker/wiseflow_usage.md and finding 2. |
| 7 | [Open-Interface](docs/non-docker/Open-Interface_usage.md) | https://github.com/AmberSahdev/Open-Interface | macOS required | Primary platform is macOS. Linux and Windows are reported with caveats. No macOS host available. See docs/non-docker/Open-Interface_usage.md. |
| 8 | [Codex-CLI](docs/non-docker/Codex-CLI_usage.md) | https://github.com/microsoft/Codex-CLI | Model retired | OpenAI retired the Codex model in March 2023. The endpoint `/v1/engines/davinci-codex/completions` returns HTTP 404. See docs/non-docker/Codex-CLI_usage.md. |
