...
## Why Not Dockerized
functionary requires 24GB or more of VRAM for its full-scale models. The research machine has an RTX 3070 with 8GB VRAM, which is insufficient. vLLM and SGLang are GPU-only runtimes with no practical CPU fallback for inference at these model sizes. Running it inside Docker also adds overhead without benefit when the blocking constraint is hardware.
