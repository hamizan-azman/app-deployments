...
- Verified: source install `pip install -r requirements.txt` succeeds in a Python 3.11 venv on Ubuntu WSL, 27 April 2026. GUI launch was not exercised. That requires WSLg, an X server, or a Windows / Linux desktop session. The dependency surface is fully resolvable.
