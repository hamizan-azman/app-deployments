...
**Deployment**: Library only. `pip install pandasai`.
...
- Use Python 3.11. The upstream supports Python 3.9 to 3.11 only. On Python 3.12 the install attempts to build pandas from source (no wheels for the version pandasai requires) and fails unless gcc, cython, meson, and pandas build headers are present. `uv venv --python 3.11` is the cleanest way to get a supported interpreter without touching the system Python.
- Verified install on Python 3.11 / pandasai 3.0.0 via `uv pip install pandasai`, 27 April 2026. `from pandasai import SmartDataframe` imported cleanly.
- For research reproducibility, pin to a specific version (`pandasai==3.0.0` or earlier `2.x`) and pass the LLM explicitly. Do not rely on env-var fallbacks.
