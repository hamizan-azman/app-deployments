...
**Deployment**: Windows-only workstation. Visible desktop session required.
...
- Verified install on Windows 10/11 + Python 3.11.9, 27 April 2026. `pip install -r requirements.txt` requires `colorama` to be installed first (`pip install colorama` before the requirements file). After that, `from ufo import ufo` imports cleanly. One package in the requirements file has a build-subprocess failure caused by missing `pkg_resources` (resolvable with `pip install --upgrade setuptools wheel` if needed), but it does not block the core import.
