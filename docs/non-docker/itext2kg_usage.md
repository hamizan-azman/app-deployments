...
**Deployment**: Library only. `pip install itext2kg`.
...
- Verified: `pip install itext2kg` resolves to v1.0.0 and `from itext2kg import iText2KG` imports cleanly on Python 3.12 (Ubuntu WSL), 27 April 2026.
- Dependency conflict (also affects `extract-thinker` and `contextgem`). `itext2kg 1.0.0` pins `openai<2.0.0,>=1.97.0`, while the other two pull in `openai>=2.x`. They cannot share a venv. Install each in its own venv if you need more than one.
