...
**Deployment**: Workstation. Cross-platform desktop GUI + FFmpeg on Linux/Win/macOS.
...
(The repository ships a `pyproject.toml`. There is no `requirements.txt` at the root.)
...
- Verified installable via PyPI (`videocaptioner==1.4.1`) and via source (`pip install .` into the cloned repo) on Python 3.11, 27 April 2026.
