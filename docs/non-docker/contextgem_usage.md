...
**Deployment**: Library only. `pip install contextgem`.
...
- Verified: `pip install contextgem` resolves to v0.22.0 and `import contextgem` works cleanly on Python 3.12 (Ubuntu WSL), 27 April 2026.
- Dependency conflict. `contextgem` pulls in `openai>=2.x` while `itext2kg 1.0.0` pins `openai<2.0.0,>=1.97.0`. They cannot share a venv. Install each in its own venv if you need both.
